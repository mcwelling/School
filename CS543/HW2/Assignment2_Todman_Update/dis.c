...
/* 4:40 PM ET on 4/22/2020  - Function to generate timestamp added by mht47@drexel.edu */

int gen_timestamp (ulong total_seconds) {
    /* Base figures*/
    ulong time_diff = total_seconds;
    ulong secs_per_day = 86400;
    ulong secs_per_hour = 3600;
    ulong secs_per_minute = 60;
    ulong init_dow = 4; /* INDEX OF INITIAL DOW - THURSDAY */
    ulong epoc = 1970;

    ulong days_since = floor( time_diff / secs_per_day );
    
    /* Hours, Minutes, Seconds */
    ulong secs_minus_days = ( time_diff % secs_per_day );
    ulong hour_of_day = floor( secs_minus_days / secs_per_hour ); /* THIS TELLS THE HOUR (24HR) */
    ulong secs_minus_days_hours = ( secs_minus_days % secs_per_hour );
    ulong minute_of_hour = floor( secs_minus_days_hours / secs_per_minute) ; /* THIS TELLS THE MINUTE */
    ulong sec_of_minute = ( secs_minus_days_hours % secs_per_minute ); /* THIS TELLS THE SECONDS */

    /* Day of Week */
    ulong dow = ( ( days_since + init_dow ) % 7 );
    char *weekdays[7] = { "Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday" };

    /* Year */
    int ref_year;
    int days_per_year[80];
	
    for( ref_year = 0; ref_year < 80; ref_year = ref_year + 1 ){
        if ( ( ref_year + epoc ) % 4 == 0 ) {
            days_per_year[ref_year] = 366; /* leap year */
        }
        else {
            days_per_year[ref_year] = 365; /* non-leap year */
        }
    }

    int year_day_counter = 0;
    int year_index = -1;

    if ( days_since >= 0 ){
        while ( year_day_counter < days_since ) {
            year_index = year_index + 1;
            year_day_counter = year_day_counter + days_per_year[year_index];
        }
    }
    else {
        year_index = 0;
    ...
    int current_year = epoc + ( year_index );   /* THIS IS THE CURRENT YEAR */
    int ly_yn;  /* THIS TELLS WHETHER LEAP YEAR */
    ...
    char *month_names[12] = { "January","February","March","April","May"
                            ...
                            ,"November","December" };
    int month_days_ly_n[12] = { 31,28,31,30,31,30,31,31,30,31,30,31 };
    int month_days_ly_y[12] = { 31,29,31,30,31,30,31,31,30,31,30,31 };
    
    int year_day_floor = 0;
    int year_day_index;
    for (year_day_index = 0; year_day_index < year_index; year_day_index = year_day_index + 1){
        year_day_floor = year_day_floor + days_per_year[year_day_index];
    }

    ulong remaining_days = days_since - year_day_floor; /* NUMBER OF DAYS INTO CURRENT YEAR */

    ulong month_day_counter = 0;
    int month_index = -1;

    if (remaining_days > 0 ) {
        if ( ly_yn > 0 ) {
            while ( month_day_counter < remaining_days ) {
                month_index = month_index + 1;
                month_day_counter = month_day_counter + month_days_ly_y[month_index];
            }
        }
        else {
            while ( month_day_counter < remaining_days ) {
                month_index = month_index + 1;
                month_day_counter = month_day_counter + month_days_ly_n[month_index];
            }
        }
    }
    else {
    ...
    int current_month = month_index;
    
    /* Day of Month */
    int month_day_floor = 0;
    int month_day_index;
    if ( ly_yn > 1 ){
        for ( month_day_index = 0; month_day_index < month_index; month_day_index = month_day_index + 1 ){
            month_day_floor = month_day_floor + month_days_ly_y[month_day_index];
        }
    }
    else {
        for ( month_day_index = 0; month_day_index < month_index; month_day_index = month_day_index + 1 ){
            month_day_floor = month_day_floor + month_days_ly_n[month_day_index];
        }
    }

    int day_of_month = remaining_days - month_day_floor;

    print( "Timestamp: %02d:%02d:%02d UTC on %s, %s %d, %d\n", hour_of_day, minute_of_hour, sec_of_minute
                                    , weekdays[dow], month_names[current_month], day_of_month, current_year );    
    return(0);
}
...
/* modified to account for updated scheduling scheme 5/7/2020 */
Prog*
delrun(int state)
{
	Prog *p;
...
vmachine(void *a)
{
	...
	/* pointers used to update process priorty in the run queue 5/6/2020 */
	Prog *pupdater;
	Prog *pfinder;
	Prog *pprev;
	...
	for(;;) {
		...
		/* Scheduling scheme modifiction 5/6/2020 */
		pfinder = isched.runhd;
		r = isched.runhd;
		while (pfinder != NULL) {
			if (pfinder->priority < r->priority) {
				r = pfinder;
			}
			pfinder = pfinder->link;
		}

		/* Move priority process to the front of the run queue if not alreadly at the front */
		if (r != isched.runhd) {
			pprev = isched.runhd;
			while (pprev-> link != r) {
				pprev = pprev->link;
		...
		if(r != nil) {
			...
			/* Priority calculator addition 5/6/2020 */
    	pupdater = isched.runhd;
    	while (pupdater != NULL){
    		ulong pscore = (((pupdater->ticks)/340)/16 + 100 + pupdater->nice); // need to update nice value 
      	if (127 < pscore) {
					pupdater->priority = 127;
				}
				else {
					pupdater->priority = pscore;
				}
				pupdater = pupdater->link;
			}

			// not needed as we are searching list for high priority item
			/*
			if(isched.runhd != nil)
			if(r == isched.runhd)

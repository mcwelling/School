...
pooldel(Pool *p, Bhdr *t)
{
	...
	// 5/26/2020 HW3 - controls for slab cases
	if(t->prev == nil) {
		return;
	}
...
pooladd(Pool *p, Bhdr *q)
{
	...
	// pointer to assist with slab navigation
	Bhdr *nav;

	q->magic = MAGIC_F;

	// diverts blocks meeting size reqs to the more granular size check
	if (q->size >= 32 && q->size <= 4096) {
		
		// looks for n s.t. 2^{n} == q
		int slnum = 5;
		while ((1 << slnum) != q->size && slnum < 13){
			slnum++;
		}
		
		// if there is a matching slab, drop q there, else fall through
		if ((1 << slnum) == q->size) {
			q->left = nil;
			q->right = nil;
			q->parent = nil;
			...
			if (slnum == 5) {
				if (p->slabs.sl32hd == nil) {
					p->slabs.sl32hd = q;
				} else {
					nav = p->slabs.sl32hd;
					p->slabs.sl32hd = q;
					p->slabs.sl32hd->fwd = nav;
				}
				//print("slab added (size 32)\n");
			} else if (slnum == 6) {
				if (p->slabs.sl64hd == nil) {
					p->slabs.sl64hd = q;
				} else {
					nav = p->slabs.sl64hd;
					p->slabs.sl64hd = q;
					p->slabs.sl64hd->fwd = nav;
				}
				//print("slab added (size 64)\n");
			} else if (slnum == 7) {
				if (p->slabs.sl128hd == nil) {
					p->slabs.sl128hd = q;
				} else {
					nav = p->slabs.sl128hd;
					p->slabs.sl128hd = q;
					p->slabs.sl128hd->fwd = nav;
				}
				//print("slab added (size 128)\n");
			} else if (slnum == 8) {
				if (p->slabs.sl256hd == nil) {
					p->slabs.sl256hd = q;
				} else {
					nav = p->slabs.sl256hd;
					p->slabs.sl256hd = q;
					p->slabs.sl256hd->fwd = nav;
				}
				//print("slab added (size 256)\n");
			} else if (slnum == 9) {
				if (p->slabs.sl512hd == nil) {
					p->slabs.sl512hd = q;
				} else {
					nav = p->slabs.sl512hd;
					p->slabs.sl512hd = q;
					p->slabs.sl512hd->fwd = nav;
				}
				//print("slab added (size 512)\n");
			} else if (slnum == 10) {
				if (p->slabs.sl1024hd == nil) {
					p->slabs.sl1024hd = q;
				} else {
					nav = p->slabs.sl1024hd;
					p->slabs.sl1024hd = q;
					p->slabs.sl1024hd->fwd = nav;
				}
				//print("slab added (size 1024)\n");
			} else if (slnum == 11) {
				if (p->slabs.sl2048hd == nil) {
					p->slabs.sl2048hd = q;
				} else {
					nav = p->slabs.sl2048hd;
					p->slabs.sl2048hd = q;
					p->slabs.sl2048hd->fwd = nav;
				}
				//print("slab added (size 2048)\n");
			} else if (slnum == 12) {
				if (p->slabs.sl4096hd == nil) {
					p->slabs.sl4096hd = q;
				} else {
					nav = p->slabs.sl4096hd;
					p->slabs.sl4096hd = q;
					p->slabs.sl4096hd->fwd = nav;
				}
				//print("slab added (size 4096)\n");
			}
			return;
		}
	}
...
dopoolalloc(Pool *p, ulong asize, ulong pc)
{
	...
	// pointer to assist with slab allocations
	Bhdr *slnav;
	...
	if (size >= 32 && size <= 4096) {
		...
		// tries to find n s.t. 2^{n} >= q
		int slnum = 5;
		while ((1 << slnum) < size){
			slnum++;
		}

		// alloc if slab corresponding to n is not empty, else fall through
		if (slnum == 5 && p->slabs.sl32hd != nil) {
			slnav = p->slabs.sl32hd;
			p->slabs.sl32hd = p->slabs.sl32hd->fwd;
			slnav->magic = MAGIC_A;
			p->cursize += slnav->size;
			if(p->cursize > p->hw)
				p->hw = p->cursize;
			unlock(&p->l);
			if(p->monitor)
				MM(p->pnum, pc, (ulong)B2D(slnav), size);
			//print("slab used (size 32)\n");
			return B2D(slnav);
		}
		if (slnum == 6 && p->slabs.sl64hd != nil) {
			slnav = p->slabs.sl64hd;
			p->slabs.sl64hd = p->slabs.sl64hd->fwd;
			slnav->magic = MAGIC_A;
			p->cursize += slnav->size;
			if(p->cursize > p->hw)
				p->hw = p->cursize;
			...
			if(p->monitor)
				MM(p->pnum, pc, (ulong)B2D(slnav), size);
			//print("slab used (size 64)\n");
			return B2D(slnav);
		}
		if (slnum == 7 && p->slabs.sl128hd != nil) {
			slnav = p->slabs.sl128hd;
			p->slabs.sl128hd = p->slabs.sl128hd->fwd;
			slnav->magic = MAGIC_A;
			p->cursize += slnav->size;
			if(p->cursize > p->hw)
				p->hw = p->cursize;
			...
			if(p->monitor)
				MM(p->pnum, pc, (ulong)B2D(slnav), size);
			//print("slab used (size 128)\n");
			return B2D(slnav);
		}
		if (slnum == 8 && p->slabs.sl256hd != nil) {
			slnav = p->slabs.sl256hd;
			p->slabs.sl256hd = p->slabs.sl256hd->fwd;
			slnav->magic = MAGIC_A;
			p->cursize += slnav->size;
			if(p->cursize > p->hw)
				p->hw = p->cursize;
			...
			if(p->monitor)
				MM(p->pnum, pc, (ulong)B2D(slnav), size);
			//print("slab used (size 256)\n");
			return B2D(slnav);
		}
		if (slnum == 9 && p->slabs.sl512hd != nil) {
			slnav = p->slabs.sl512hd;
			p->slabs.sl512hd = p->slabs.sl512hd->fwd;
			slnav->magic = MAGIC_A;
			p->cursize += slnav->size;
			if(p->cursize > p->hw)
				p->hw = p->cursize;
			...
			if(p->monitor)
				MM(p->pnum, pc, (ulong)B2D(slnav), size);
			//print("slab used (size 512)\n");
			return B2D(slnav);
		}
		if (slnum == 10 && p->slabs.sl1024hd != nil) {
			slnav = p->slabs.sl1024hd;
			p->slabs.sl1024hd = p->slabs.sl1024hd->fwd;
			slnav->magic = MAGIC_A;
			p->cursize += slnav->size;
			if(p->cursize > p->hw)
				p->hw = p->cursize;
			...
			if(p->monitor)
				MM(p->pnum, pc, (ulong)B2D(slnav), size);
			//print("slab used (size 1024)\n");
			return B2D(slnav);
		}
		if (slnum == 11 && p->slabs.sl2048hd != nil) {
			slnav = p->slabs.sl2048hd;
			p->slabs.sl2048hd = p->slabs.sl2048hd->fwd;
			slnav->magic = MAGIC_A;
			p->cursize += slnav->size;
			if(p->cursize > p->hw)
				p->hw = p->cursize;
			...
			if(p->monitor)
				MM(p->pnum, pc, (ulong)B2D(slnav), size);
			//print("slab used (size 2048)\n");
			return B2D(slnav);
		}
		if (slnum == 12 && p->slabs.sl4096hd != nil) {
			slnav = p->slabs.sl4096hd;
			p->slabs.sl4096hd = p->slabs.sl4096hd->fwd;
			slnav->magic = MAGIC_A;
			p->cursize += slnav->size;
			if(p->cursize > p->hw)
				p->hw = p->cursize;
	...
	if(q != nil) {
		...
		if(frag < (size>>2) && frag < 0x8000) {
			...
			//print("tree best fit alloc (size %d)\n", q->size);
			return B2D(q);
		}
		/* Split */
		ns = q->size - size;
...
poolfree(Pool *p, void *v)
{
	...
	// prevent coalescing of potential slab blocks
	if(b->size > 4096) {
		c = B2NB(b);
